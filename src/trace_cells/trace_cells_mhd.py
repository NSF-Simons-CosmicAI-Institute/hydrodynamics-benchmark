...
def main(path,mass,radius):
    ...
    for ind in range(0,len(data[0,:,6])):
        density_current = data[:,ind,6]
        if np.any(density_current > rho0/200.):
    ...
    # Filter cells based on zero photon energy bug
    ind_uv_filter = []
    for ind in range(0,len(data[0,:,11])):
        uv_current = data[:,ind,11]
        if any(uv_current == 0):
            continue
        else:
            ind_uv_filter.append(ind)
    data = data[:,ind_uv_filter,:]
    print('Cells retained after UV filter: ' + str(len(ind_uv_filter))) 
